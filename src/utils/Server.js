        ...
        const withComplete = (...fns) => {
            return (res) => {
                if (process.env.NODE_ENV === 'development') {
                    console.log('%crequest%c ' + url + ' %cresult:', 'border-radius:2px;padding:0 2px;background:blue;color:#fff', 'background:rgb(220, 242, 253);color: rgb(97, 140, 160)', 'background-color: rgb(220, 242, 253); color: rgb(97, 140, 160);', res);
                }

                isShowLoading && loader.hideLoader();
                fns.reduce((prev, fn) => fn(prev), res);
            };
        };
